  ...
  parsed_clusters <- lapply(cluster_blocks, function(block) {
    tibble::tibble(
      survey_area = stringr::str_extract_all(basename(txt[[area_name]]), "^[^.]+") |>
        as.character(),
      nr_EAs = stringr::str_extract(txt[[18]], "\\d+"),
      total_children = stringr::str_extract(txt[[19]], "\\d+"),
      total_cases = stringr::str_extract(txt[[20]], "\\d+"),
      `%_cases` = stringr::str_extract(txt[[21]], "\\d+"),
      ...
      children = stringr::str_extract(block[4], "[0-9]+"),
      n_cases = stringr::str_extract(block[5], "[0-9]+"),
      expected_cases = stringr::str_extract(block[6], "[0-9]+[.]+[0-9]+"),
      observedExpected = stringr::str_extract(block[7], "[0-9]+[.]+[0-9]+"),
      relative_risk = stringr::str_extract(block[8], "[0-9]+\\.\\d+"),
      `%_cases_in_area` = stringr::str_extract(block[9], "[0-9]+\\.\\d+"),
      log_lik_ratio = stringr::str_extract(block[10], "[0-9]+\\.\\d+"),
      pvalue = stringr::str_extract(block[11], "[0-9]+[.]+[0-9]+"),
      ipc_amn = ifelse(
